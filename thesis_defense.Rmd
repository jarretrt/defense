...
use_freezeframe(trigger = "click", overlay = TRUE)
```
---
# Overview
...
- Within categorical regions, take Taylor series approximation as a function of the data at the means of each variable, $(\mu_1,...\mu_Q)$
...
- **Procedure:** Group data into local regions and construct squashed sample to have the same moments as the original data within each region
...
- Define regions to have constant missingness patterns and evaluate Taylor series on the observed data likelihood
...
In each region, numeric variables will be either fully observed or fully missing 

3. Create squashed data set with the same missingness patterns as the original data set
...
- Moment-matching occurs in local regions (i.e., within constant categories, numeric clusters) for accuracy of Taylor approximation
...
.center[
```{r, squash-analysis-2-fp, out.width = 745, out.height = 595}
